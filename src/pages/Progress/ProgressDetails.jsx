...
const ProgressDetails = () => {
    ...
    const getProgressData = async () => {
        try {
            ...
            setImages(data.progressData.media != null ? data.progressData.media : []);
            setProgres(data.progressData);
            setStatus(data.progressData.status);

            let userids = data.progressData.comments?.map(({user_id}) => user_id);
            let {data: usersData} = await  getUsersListByIds({
                ids: userids
            });
            console.log({usersData});
            ...
            let newComments = data.progressData.comments.map((com , index )=> {
                let name = usersData.find(({id}) => id === com.user_id)?.firstname;
                return {...com , userName : name}
            });
                    ...
                    <div className="mt-6">
                        ...
                        <div className="space-y-4">
                            {comments.map((comment, index) => (
                                <div
                                    key={index}
                                    className={`flex items-start space-x-3 ${comment.user_id === user.id ? 'justify-end' : ''}`}
                                >
