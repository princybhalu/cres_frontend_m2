...
const schema = yup.object().shape({
  ...
  resouces_type: yup.string().required('Resoucre Type is required'),
  image: yup
    .mixed()
...
const AddResourceForm = ({onClose}) => {
  ...
  const onSubmit = async (data) => {
    ...
    let requestBody = {
      ...
      resources_type: data.resouces_type,
      media: [],
      type: "resource",
      user_id: user.id
    };
    ...
    try {
      ...
      navigate("/project/"+ projectId + "/resource");
      onClose();
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='mt-3'>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-1">
            ...
            <input
              type="text"
              placeholder="Enter Resoucre Type"
              {...register('resouces_type')}
              className={`w-full p-2 border ${errors.resouces_type ? 'border-red-500' : 'border-gray-300'} rounded`}
            />
            {errors.resouces_type && <p className="text-red-500 text-sm">{errors.resouces_type.message}</p>}
          </div>
